-- Drop and recreate function to also search in admins table
...
BEGIN
  ...
  -- 3. admins.cpf (raw digits or formatted)
  -- 4. Direct UUID match on profiles.id
  -- 5. Direct UUID match on client_ai_memory.user_id
  ...
  IF v_user_id IS NULL THEN
    ...
    WHERE a.cpf = identifier
       OR replace(replace(a.cpf, '.', ''), '-', '') = identifier
  ...
  IF v_user_id IS NULL THEN
    BEGIN
      -- Check if a memory record exists for this UUID
      SELECT cam.user_id INTO v_user_id
      FROM public.client_ai_memory cam
      WHERE cam.user_id = identifier::uuid
      LIMIT 1;
...
-- Also update admin record to link to the correct user
UPDATE public.admins 
SET user_id = '810f7a82-1bd7-4999-86a1-ddb4194b7254'
WHERE cpf = '10533219531' AND user_id IS NULL;